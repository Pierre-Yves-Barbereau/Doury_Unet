...
model = rnn.UNet_Doury(1,1).to(device)
...
plt.legend()
plt.title('Test loss last 100 it')
plt.xscale("log")
plt.yscale("log")
plt.grid(True)
plt.show()
...
if save == 'y':
    ...
    file.write(f"%s \nInput_channels = %s \nOutput_channels = %s \nLoss = MSE \nMSE_mean_last_100_it = %s \nEPOCHS = %s \nBatch_size = %s\nLr_init = %s \nSheduler_period = %s \nTraining time = %s\nOpt = %s"%(dir_name,input_channels,input_channels[output_channel_indice],loss_test_mean_100,EPOCHS,batch_size,lr,sheduler_period,training_time,opt))
    file.close()
    ...
    torch.save(model.state_dict(), f"%s/Models.py" %(dir_name))
