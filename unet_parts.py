...
class updc(nn.Module):
    """Double conv then Upscaling then double conv"""
...
class updcout(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        # if bilinear, use the normal convolutions to reduce the number of channels
        if bilinear:
            self.up = nn.Upsample(size=(214,267), mode='bilinear', align_corners=True)
            self.dc = DoubleConv(in_channels, out_channels, in_channels // 2)
        else:
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.dc = DoubleConv(in_channels, out_channels)
